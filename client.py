...
############################## SCRIPT ENTRY POINT ##############################
################################################################################
from config import grab_config
from itertools import cycle, islice

selfIp, hosts, ports, addrs, nets = grab_config()

def round_robin(protocol, requestno):
    for addr, port in islice(cycle(zip(addrs, ports)), requestno):
        http_get(f'{protocol}://{addr}:{port}')
    pass

def fastest_response_time(protocol, requestno):
    pass

def random_choice(protocol, requestno):
    pass
